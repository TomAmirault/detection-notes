...
def double_alignment(sourcea, sourceb):
    ...
    # Détection des keypoints
    orb = cv2.ORB_create(nfeatures = nfeaturesORB2)
    keypoints1, descriptors1 = orb.detectAndCompute(img1_2, None)
    keypoints2, descriptors2 = orb.detectAndCompute(img2_2, None)
    ...
    cv2.imshow("Image1 - keypoints pour second alignement", img1_2_with_keypoints)
    ...
    all_matches = list(matcher.match(descriptors1, descriptors2, None))
    sorted_matches = sorted(all_matches, key=lambda x: x.distance, reverse=False)
    numGoodMatches = int(len(sorted_matches) * topMatchesFactor2)
    matches = sorted_matches[:numGoodMatches]
...
double_alignment(sourcea, sourceb)